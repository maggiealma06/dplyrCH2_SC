...
#or you could do
hflights |>
  select(ActualElapsedTime:DepDelay)
#if collums are consecutive
...
select(hflights, -(DepTime:AirTime)) #this one gets rid of the chunks in between rather than listing the variables you want
...
select(hflights, "TaxiIn", "TaxiOut", "Distance") |> head(6)
...
g2 <- mutate(g1, GroundTime = TaxiIn + TaxiOut) |> relocate(GroundTime, .after = ActualGroundTime) 
```
...
m2 <- mutate(hflights, loss = ArrDelay - DepDelay, loss_percent = loss / DepDelay * 100) |>
  relocate(loss_percent, .after = Year) 
head(m2)
```
